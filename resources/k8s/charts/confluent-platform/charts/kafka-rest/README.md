...
### Install with a existing kafka and schema-registry release

```console
helm install --set zookeeper.url="unhinged-robin-zookeeper:2181",schema-registry.url="lolling-chinchilla-schema-registry:8081" cp-helm-charts/charts/kafka-rest
```
...
hopping-salamander-kafka-rest  ClusterIP  10.19.250.118  <none>       8082/TCP  1m

==> v1beta2/Deployment
NAME                              DESIRED  CURRENT  UP-TO-DATE  AVAILABLE  AGE
hopping-salamander-kafka-rest  1        1        1           1          1m

==> v1/Pod(related)
NAME                                               READY  STATUS   RESTARTS  AGE
hopping-salamander-kafka-rest-67b86cff98-qxrd8  1/1    Running  0         1m

==> v1/ConfigMap
NAME                                            DATA  AGE
hopping-salamander-kafka-rest-jmx-configmap  1     1s
```

There are
1. A [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) `hopping-salamander-kafka-rest` which contains 1 REST Proxy [Pod](https://kubernetes.io/docs/concepts/workloads/pods/pod-overview/): `hopping-salamander-kafka-rest-67b86cff98-qxrd8`.
1. A [Service](https://kubernetes.io/docs/concepts/services-networking/service/) `hopping-salamander-kafka-rest` for clients to connect to REST Proxy.
...
1. (Optional) A [Service](https://kubernetes.io/docs/concepts/services-networking/service/) `hopping-salamander-kafka-restproxy-external` for clients to connect to REST Proxy from outside.
...
```console
helm install --name my-rest-proxy -f my-values.yaml ./kafka-rest
```
...
The configuration parameters in this section control the resources requested and utilized by the `kafka-rest` chart.
...
| Parameter | Description | Default |
| --------- | ----------- | ------- |
| `image` | Docker Image of Confluent REST Proxy. | `confluentinc/kafka-rest` |
...
| Parameter | Description | Default |
| --------- | ----------- | ------- |
| `zookeeper.url` | Service name of Zookeeper cluster (Not needed if this is installed along with kafka chart). | `""` |
...
| Parameter | Description | Default |
| --------- | ----------- | ------- |
| `schema-registry.url` | Service name of Schema Registry (Not needed if this is installed along with kafka chart). | `""` |
| `schema-registry.port` | Port of Schema Registry Service | `8081` |
